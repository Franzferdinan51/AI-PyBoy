...
const ScreenOverlay: React.FC<{ aiState: AIState }> = ({ aiState }) => {
  ...
  switch (aiState) {
    ...
    case AIState.THINKING:
      text = 'AI Thinking...';
      color = 'text-cyan-glow animate-pulse';
      break;
    case AIState.ERROR:
      text = 'AI Error';
      color = 'text-red-500';
  ...
  return (
    <div className="absolute inset-0 bg-black/50 flex items-center justify-center pointer-events-none">
      <p className={`font-display text-2xl font-bold tracking-widest ${color}`}>{text}</p>
    </div>
  );
};
