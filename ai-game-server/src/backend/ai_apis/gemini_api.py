...
class GeminiAPIConnector(AIAPIConnector):
    ...
    def __init__(self, api_key: str):
        ...
        self.model = "models/gemini-1.5-pro-latest"  # Default Gemini model
    ...
    def get_models(self) -> List[str]:
        ...
        try:
            ...
            self.logger.error(f"Failed to fetch models from Gemini API: {e}")
            return ["models/gemini-1.5-pro-latest", "models/gemini-pro-vision", "models/gemini-pro"]
    ...
    def get_next_action(self, image_data: bytes, goal: str, history: List[str]) -> str:
        ...
        try:
            ...
            # Create the prompt
            prompt = f"""You are an expert AI playing a retro video game. Your current high-level objective is: "{goal}".
Review the last few actions taken: {', '.join(history[-5:])}.
